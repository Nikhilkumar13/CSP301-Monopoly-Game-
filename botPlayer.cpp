...
/*Non-default constructor, read data from external source and assigns values to the data members*/
 botPlayer::botPlayer(double start_money, string& player_name,bool bot ){
    money = start_money;
    space = 1;
    for (int i (0); i < 40; i++){
  ...
  char  botPlayer::play() const
{
     ...
     cout<<"(M)ortgage"<<endl;
     cout<<"I would Like to roll"<<endl;
     // cin>>choice;
     choice='r';
     return choice;
}
...
/*Member function for if the  botPlayer owns the indicated space*/
  bool  botPlayer::own_property(const int index) const{
    return (properties_owned[index]);
}
/*Member function that returns the  botPlayer's name*/
  string  botPlayer::getName () const{
    return (name);
}
/*Member function that returns if the  botPlayer owns a GooJF card*/
  bool  botPlayer::getGooJF () const{
    return (GooJF);
}
/*Member function that returns if the  botPlayer is in jail*/
  bool  botPlayer::getJail () const{
    return (jail);
}
/*Member function that returns the amount of money in the  botPlayer's account*/
  double  botPlayer::getMoney () const{
    return (money);
}
/*Member function for if the  botPlayer owns an entire monopoly*/
  bool  botPlayer::own_monopoly(const int index) const{
    if (index == 1){
...
/*Member function that returns the number of railroads the  botPlayer owns*/
  int  botPlayer::RR_owned () const{
    int counter (0);
...
/*Member function that returns the number of utilities the  botPlayer owns*/
  int  botPlayer::utilities_owned () const{
    int counter (0);
...
/*Member function that returns the  botPlayer's assets*/
  double  botPlayer::getAssets(Properties props[]){
    double assets(money);
    for (int n (0); n < 40; n++){ //Calculate the  botPlayer's assets
...
/*Member function that subtracts money from the  botPlayer's account*/
  void   botPlayer::subMoney (const double change){
    money -= change;
}
/*Member function that adds money to the  botPlayer's account*/
  void  botPlayer::addMoney (const double change){
    money += change;
}
/*Member function for buying and selling properties*/
  void  botPlayer::setProperty (const int index, const bool status){
    properties_owned[index] = status;
}
/*Member function to give or take away GooJF cards*/
  void  botPlayer::setGooJF (const bool status){
    GooJF = status;
}

/*Roll member function, moves the character*/
  void  botPlayer::roll (const int index, Properties props[], Chest chest_cards[], Chance chance_cards[], const bool roll_again, const bool jackpot,
    ...
    cout<<endl;
    if (jail){ //If the  botPlayer is in jail
        doubles = 0;
        char response; //Variable for the  botPlayer input
    ...
    cout<<name<<" landed on "<<props[space-1].getName()<<"."<<endl; //Print out the name of the space they landed on
        
    
    /*Declare variables for use inside of switch statement*/
    /*Income tax*/
    double assets (money); //Total assets of this  botPlayer
    /*Chance / Community Chest*/
    int chest_card_num (rand() % num_chest); //Chooses a random card
    int chance_card_num (rand() % num_chance); //Chooses a random card
    int new_space (0); //Holds a new space for the  botPlayer to go to
    double money_bank (0), money_players (0); //Money that goes either to the bank or to other players
    
    /*Execute results based on what space the  botPlayer lands on*/
    ...
    switch (space){
        ...
        case 21 : //Free parking
            if (jackpot){ //If there is a jackpot, print out the worth and add it to the  botPlayer's bank
        ...
        case 31 : //Go to Jail
            jail = true;
            space = 11; //Send the  botPlayer to jail
        ...
        case 34:
            ...
            if (new_space > 0){ //If the  botPlayer moved
                if (new_space == 11){
                    space = new_space; //If the  botPlayer was sent to jail
                    jail = true;
                    break;
                }
                if (new_space < space){
                ...
                space = new_space; //Move the  botPlayer to the new space
            ...
            if (new_space > 0){ //If the  botPlayer moved
                ...
                if (!(props[space - 1].getOwned () ) ){ //If the property is not owned
                    ...
                    if (money - props[space-1].getPrice() > 0){
                        cout<<"Would you like to purchase "<<props[space-1].getName()<<" for $"<<props[space-1].getPrice()<<"? (Y/N)"<<endl;
                        while (true){
                        	//----Purchase Property or Not Logic Bot--------------------------
                            // cin>>response;
                            if(money>200)
                            {
                            	response='Y';
                            ...
                            //-----------------end--------------------------------------------
                            if (response == 'Y' || response == 'y'){ //The  botPlayer chooses to purchase the property
                                money -= props[space-1].getPrice();
                                props[space-1].setOwned (true);
                                properties_owned[space-1] = true;
                                cout<<"You now own "<<props[space-1].getName()<<"!"<<endl;
                            ...
                            else if (response == 'N' || response == 'n'){ //The  botPlayer does not want to purchase the property
                                cout<<"You did not purchase "<<props[space-1].getName()<<"."<<endl;
                                break;
                            }
                            else{
                    ...
                    else{ //The  botPlayer does not have enough money to purchase the property
                        cout<<"You do not have enough money to purchase "<<props[space-1].getName()<<"."<<endl;
                    }
                }
                ...
                switch (exception){ //Used for complicated directions
                    ...
                    case 1 : //Nearest RR
                        ...
                        if (!(props[space - 1].getOwned () ) ){ //If the property is unowned, allow the  botPlayer to purchase it
                            char response;
                            cout<<"This property is unowned."<<endl;
                            if (money - props[space-1].getPrice() > 0){
                                cout<<"Would you like to purchase "<<props[space-1].getName()<<" for $"<<props[space-1].getPrice()<<"? (Y/N)"<<endl;
                    ...
                    case 2 : //Nearest utility
                        ...
                        if (!(props[space - 1].getOwned () ) ){ //If the property is unowned, allow the  botPlayer to purchase it
                            char response;
                           string pName=props[space-1].getName();
                            cout<<"This property is unowned."<<endl;
                            if (money - props[space-1].getPrice() > 0){
                                cout<<"Would you like to purchase "<<pName<<" for $"<<props[space-1].getPrice()<<"? (Y/N)"<<endl;
                                while (true){
                                	//--------------------BOt logic for purchasing Property---------------
                    	if(money>200)
                    	{
                    		cout<<"Yes I Will Purchase "<<pName<<endl;
                    		response='Y';
                    	}
                    	else
                    	{
                    		cout<<"No,Not at all"<<endl;
                    		response='Y';
        ...
        case 36:
            if (!(props[space - 1].getOwned () ) ){ //If the property is unowned, allow the  botPlayer to purchase it
                ...
                if (money - props[space-1].getPrice() > 0){
                	string pName =props[space-1].getName();
                    cout<<"Would you like to purchase "<<name<<" for $"<<props[space-1].getPrice()<<"? (Y/N)"<<endl;
                    while (true){
                    	//--------------------BOt logic for purchasing Property---------------
                    	if(money>200)
                    	{
        ...
        default :
            if (!(props[space - 1].getOwned () ) ){ //If the property is owned, allow the  botPlayer to purchase it
                char response;
                cout<<"This property is unowned."<<endl;
               string  pName=props[space-1].getName();
                if (money - props[space-1].getPrice() > 0){
    ...
    /*Check if the  botPlayer rolled doubles and the game settings*/
...
/*Buy member function, used to buy and sell houses*/
  void  botPlayer::buy(Properties props[]){// botPlayer buy function
    cout<<endl<<name<<", you own the following unmortgaged properties:"<< endl;//Show the  botPlayer their unmortgaged properties
    for (int i(0); i < 40; i++){
    ...
    while(flag){
    cout << endl << "Would you like to (B)uy or (S)ell? " << endl << "Enter '0' to return to the main menu." << endl;// ask if the  botPlayer wants to buy or sell.
    cin >> input;
    if( input == '0'){
        return;
    }
    if( input == 'B' || input == 'b' ){
        ...
        if(!(response==5||response==15||response==25||response==35||response==12||response==28)){
            if(properties_owned[response]){
                if (own_monopoly(props[response].getID())){//Checks to see that the  botPlayer owns the monopoly
                    if(props[response].getHouses() < 5){//Checks to make sure they can still build
                        ...
                        for(int i(0); i< 40; i++){
                            if(props[response].getID() == props[i].getID()){
                                if(!(props[i].getHouses() == num_Houses || props[i].getHouses() == (num_Houses + 1))){//checks to see if the  botPlayer is building evenly
                        ...
                        if(flagMon){//If the are building evenly allow them to buy
                            char input2;
                            cout << "The house you are about to buy costs $" << props[response].getH_cost() << "." << endl;//Present the  botPlayer with the cost.
                            ...
                            while(flagYN){
                                ...
                                if( input2 == 'Y' || input2 == 'y' ){
                                    ...
                                    if (money - props[response].getH_cost() < 0){
                                        ...
                                        subMoney(props[response].getH_cost());//takes money from  botPlayer
            ...
            if(properties_owned[response]){//check to see that they own the property
                if(props[response].getHouses() >= 1){//checks to see that they have houses to sell
                    int num_Houses = props[response].getHouses();//stores the number of houses into int
                        ...
                        for(int i(0); i< 40; i++){
                            if(props[response].getID() == props[i].getID()){
                                if(!(props[i].getHouses() == num_Houses || props[i].getHouses() == (num_Houses - 1))){//checks to see if the  botPlayer is selling evenly
                        ...
                        if(flagMon){
                            char input2;
                            cout << "You will receive $" << props[response].getH_cost() << " for selling your house." << endl;//Present the  botPlayer with the cost.
                            bool flagYN(true);
                            while(flagYN){
                                ...
                                if( input2 == 'Y' || input2 == 'y' ){
                                    flagYN=false;
                                    addMoney(props[response].getH_cost());//gives money from  botPlayer
...
/*Stats member function, prints out the  botPlayer's stats*/
  void  botPlayer::stats (Properties props[]) {
    cout << endl << name << ", you have $" << money << "." << endl << endl; // Print out the name of the  botPlayer and the amount of money the  botPlayer has
    cout << "You own the following properties: " << endl; // Print out a list of the properties the  botPlayer owns
    for (int i=0; i<40; i++){
        if (properties_owned[i]){ // Check to see what properties the  botPlayer owns
            ...
            if (props[i].getHouses() > 0) { //Check to see if there are any houses on any of the properties
                cout<<"\t(";
                if (props[i].getHouses() == 5) { //If the  botPlayer owns a hotel, print it out to the screen
                    cout << "Hotel)" << "\t";
                }
                else{ //If the  botPlayer owns hoses, print out how many
            ...
            } //Inform the  botPlayer if the property is mortgaged
    ...
    cout << endl;
    /*Print out if the  botPlayer owns a monopoly*/
    if (own_monopoly (1)){
    ...
    cout << endl;
    if (jail){ //Specify if the  botPlayer is in jail
...
/*Trading member function, only prints out data to  botPlayer*/
  void  botPlayer::trade(Properties props[]){
    cout<<endl<<name<<" has $"<<money<<"."<<endl; //Tell the  botPlayer how much money they have
    cout<<"They also own the following properties:"<<endl; //List off what properties the  botPlayer owns
    for (int i(0); i < 40; i++){
    ...
    if(GooJF){ //Print out if they  botPlayer owns a GooJF card
        cout<<name<<" has a Get out of Jail Free Card."<<endl;
    }
}

/*Mortgage member function*/
  void  botPlayer::mortgage(Properties props[]){
    cout<<endl<<name<<", you have $"<<money<<"."<<endl; //Tell the  botPlayer how much money they have
    cout<<"You own the following properties:"<<endl; //List off what properties the  botPlayer owns
    for (int i(0); i < 40; i++){
        if(properties_owned[i]){ //If they own the property, list it
    ...
    while(true){ //Mortgage loop
        while(!(cin>>response)){ //Ensure a valid input
            cin.clear();
            getline(cin, trash, '\n');
            cout<<"Invalid input. Please enter one of the  botPlayer numbers."<<endl;
        }
        while(response >= 40){ //Ensure that the number is less than 40
            response -= 40;
        }
        ...
        if(properties_owned[response]){ //If they own that property
            ...
            if (!(flag)){ //If they do not have houses, mortgage/unmortgage the property
                if(props[response].getMortgaged ()){
                    ...
                    if (money - (value * 1.1) < 0){ //Ensure that the  botPlayer has enough money to unmortgage the property
                        cout<<"You do not have enough money to unmortgage "<<props[response].getName ()<<"."<<endl<<endl;
                    }
                    else{ //If they  botPlayer has enough money, unmortgage the property
                        money -= (value * 1.1);
...
/*Member function for debt*/
  void  botPlayer::debt(Properties props[]){
    if (money > 0){ //If they still have money left, nothing changes
        ...
        if (abs(money) > assets){ //If they do not have enough money to pay off the debt, they lose
            ...
            int response; 
            while (money <= 0){ //While the  botPlayer is still in debt
                while(!(cin>>response)){
                    string trash;
                    cin.clear();
                    getline(cin, trash, '\n');
                ...
                if (properties_owned[response]){ //Ensure the  botPlayer ownes the property
                    if (props[response].getHouses() > 0){ //If there are houses
                        int id = props[response].getID();
                        int num_houses = props[response].getHouses();
                        bool flag (true); 
                        for(int n (0); n < 40; n++){ //Make sure the  botPlayer is selling houses evenly
                            if(props[n].getID() == id){
